...
export const createCreature = async (req, res) => {
  try {
    ...
    if (!name || !origin) {
      return res
        .status(400)
        .json({ error: "Le nom et l'origine sont requis." });
    }

    const existing = await Creature.findOne({ name });
    if (existing)
      return res.status(400).json({ error: "Ce nom de créature existe déjà." });
...
export const createTestimony = async (req, res) => {
  try {
    ...
    if (!creatureId || !description) {
      return res
        .status(400)
        .json({ error: "ID de créature et description obligatoires." });
    }
    ...
    if (alreadyPosted) {
      return res
        ...
        .json({
          error:
            "Vous avez déjà posté un témoignage récemment. Attendez 5 minutes.",
        });
    }
...
export const validateTestimony = async (req, res) => {
  try {
    ...
    if (String(testimony.authorId) === String(req.user.id)) {
      return res
        .status(403)
        .json({ error: "Vous ne pouvez pas valider votre propre témoignage." });
    }
